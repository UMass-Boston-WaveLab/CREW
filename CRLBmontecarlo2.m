...
for ii=1:length(testM)
    for jj = 1:reps
        [r, crlb]=chanestCRLB(N, testM(ii), d, q, 10^(SNR/10));
        temp=temp+crlb/reps;
...
for ii=1:length(testN)
    for jj = 1:reps
        [r, crlb]=chanestCRLB(testN(ii), M, d, q, 10^(SNR/10)); %averaging
        temp=temp+crlb/reps;
...
for ii=1:length(testd)
    for jj = 1:reps
        [r, crlb]=chanestCRLB(N, M, testd(ii), q, 10^(SNR/10));
        temp=temp+crlb/reps;
...
for ii=1:length(testq)
    for jj = 1:reps
        [r, crlb]=chanestCRLB(N, M, d, testq(ii), 10^(SNR/10));
        temp=temp+crlb/reps;
...
for ii=1:length(testSNR)
    for jj = 1:reps
        [r, crlb]=chanestCRLB(N, M, d, q, 10^(testSNR(ii)/10));
        temp=temp+crlb/reps; 
