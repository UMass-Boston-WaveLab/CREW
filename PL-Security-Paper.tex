...
%belongs to Eric
Since the channel's spatial variation is not ergodic, it is well suited to long-range prediction techniques (someone, perhaps a duel-hallen paper, said this). rather than using long-range prediction to predict the channel transfer function ahead temporally, we predict it spatially.  %say what we do and why (spectral estimation is ok because  parameters don't change very fast)
...
These prediction lengths are unrealistic because the simulated channel doesn't change vs. position at all.  A realistic channel will change as the eavesdropper's position changes.  In temporal prediction applications, it has been observed that time variation of the amplitudes and frequencies in the sum-of-sinusoids model is on the order of 0.1 seconds \cite{duel-hallen2007} \emph{This may depend on how fast they assume the receiver is moving?}.  Based on this observation, we expect that \emph{something something}.
