...
%belongs to Clara
To be able to connect two parties in an encrypted channel without needing predetermined keys would allow for ease of communication, but require a common source of random information that is unique to both communicating parties. It has been demonstrated \cite{azimisadjadi2007, bloch2008, mathur2008, ye2010} that two parties' reciprocal observations of wireless channel fading can be used as a common source of randomness to generate symmetric encryption keys on the fly in a situation where two parties cannot pre-arrange keys.  
...
Recently proposed fading-based encryption methods \cite{mathur2008, ye2010} assume that 
%Previously it was assumed that
the channel observed of an eavesdropper farther than approximately a carrier wavelength away from the communicating nodes have almost no mutual information with the channel observed by the communicating nodes, since the channel correlation function falls off very quickly with distance.
 ...
 This assumption stemming from the idea that the channel model between A and B's location are random. %fix grammar, cite Jakes, depends on channel being Gaussian (because correlation function is proportional to mutual information for Gaussian random variables) - random is the wrong word choice here
...
However, it has been shown that real-world wireless channels are typically not ergodic vs. time \cite{isukapalli2006}.  In this work, we show that security claims based on the correlation length are inappropriate.  The spatial average of a non-ergodic channel's correlation function is not equal to the ensemble average, meaning that the channel is also not necessarily ergodic vs. space over a given sampling region.  Since that a new channel realization is not obtained within the sampling region, consecutive samples of the same channel are deterministically related to each other by the physical parameters of the channel. To develop a more conservative and practical estimate of the minimum distance from a node to an eavesdropper, we apply channel prediction techniques \cite{duel-hallen2007} to investigate the distance over which it is possible to predict the channel transfer function.
